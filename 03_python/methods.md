...
- s.find(x)와 같이, 문자열에서 처음 나오는 x의 위치를 반환한다.
...
- s를 검증하는 method이므로 ()안에는 아무것도 들어가지 않는다.
...
#### separator.join(iterable[객체])

- iterable의 인자들 사이에 separator를 넣어서 **문자열을 반환한다.**
...
#### 이외에 .capitalize(), .title(), .upper(), .lower(), .swapcase() 등이 있다.
...
- list의 정해진 위치 index에 x값을 추가한다.
- i가 리스트의 길이보다 긴 경우, 리스트의 맨 뒤에 값을 추가한다.
